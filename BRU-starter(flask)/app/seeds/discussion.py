...
def seed_discussions():
    ...
    def auto_seed(count, user_count, discussion_count):
        for i in range(count):
            user_id = random.randint(1, user_count)
            discussion_id = random.randint(1, discussion_count)
            body = fake.paragraph(nb_sentences=random.randint(1, 5))
            created_at = datetime.now()
            seed_message = Reply(userId=user_id, discussionId=discussion_id, body=body, created_at=created_at)
